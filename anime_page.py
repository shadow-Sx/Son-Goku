...
def send_anime_page(chat_id: int, code: int):
    ...
    elif anime.get("video_id"):
        bot.send_video(chat_id, anime["video_id"], caption=caption, reply_markup=kb)
    else:
...
def open_episode(call):
    ...
    bot.send_video(call.message.chat.id, ep["file_id"], caption=f"{number}-qism")
